function [lat,lon,dep,T,M]=parseCorrea(filename,lat_L,lon_L,dep_L,T_L,M_L)
  % Simple routine to parse csv files from PHD.
  
  % Load in the EQ data.
  % yyyy, mm, dd, hh, mm, ss, lat, lon, dep, mag
  command=['cat ', filename, ' | ', ... % Load in all the EQ data files.
           'awk -F,  ''/ORIGIN/{next} {print $1, $2, $3, $4, $7 }'' | ' ... % Grab just the necessary fields.
  ...
  T=datetime(data(:,1),data(:,2),data(:,3),data(:,4)-3,data(:,5),data(:,6));
  lat=data(:,7);
  lon=data(:,8);
  dep=data(:,9);
  M=data(:,10);
